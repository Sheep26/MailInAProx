async function loadSend() {
    const emails_req = await fetch('/api/get_sent');
    const emails = await emails_req.json();

    const inbox = document.getElementById("sent");
    ...
    for (let email of emails) {
        ...
        element.onclick = function() {window.location = `/view_sent_email?sent_id=${email.sent_id}`};

        element.innerHTML = `
        <div class="flex column">
            <span>${email.mail_to}</span>
